...
10. During rename mode the entity type icon remains visible to the left of the text input; the input is borderless, fills the remaining row width, and matches the outliner's default font size so the row height does not change when entering or exiting rename mode.
...
- The text input must be rendered as borderless (`TextEdit::frame(false)`) filling the remaining row width (`desired_width(f32::INFINITY)`); the entity type icon must remain visible to the left via `ui.label(icon)` inside a `ui.horizontal` block; the row height must not change when entering or exiting rename mode.
...
3. When `renaming_index == Some(index)`, render the row as `ui.horizontal` with `ui.label(icon)` followed by `egui::TextEdit::singleline(&mut name).frame(false).desired_width(f32::INFINITY).show(ui).response`; request focus on the first frame using `response.request_focus()`.
